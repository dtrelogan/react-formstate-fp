...
import { convertToRootModelKey, getId, isScope, buildValidationSchema } from './schemaAndLookup.js';
...
function createElement(elementName, props) {
  ...
  if (required || validate || validateAsync) { // This is really just to shut up eslint about unused variables.
    ...
    if (Object.keys(jsxValidationSchema).length > 0) {
      if (!form.validationSchemas) {form.validationSchemas = {};}
  ...
  return React.createElement(
    elementName === 'Form' ? 'form' : React.Fragment,
    otherProps,
...
function validateNameRequired(props, isFormScope) {
  if (!hasProp(props, 'name') || (!isNonEmptyString(props.name) && typeof(props.name) !== 'number')) {
    throw new Error(`A ${isFormScope ? 'nested FormScope' : 'FormField'} element requires a non-empty "name" prop.`);
  }
}
